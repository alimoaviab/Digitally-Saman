...
export default function Navbar() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Scroll active state check
      const sections = ['about', 'services', 'why-me', 'process', 'contact'];
  ...
  const handleNavClick = (e: React.MouseEvent<HTMLAnchorElement>, href: string) => {
    ...
    // Smooth scroll inside home page
    ...
    <>
      <motion.nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
          isScrolled 
            ? 'py-4 bg-[#03001e]/70 backdrop-blur-md border-b border-white/5 shadow-[0_4px_30px_rgba(0,0,0,0.5)]' 
          ...
          <div className="hidden lg:flex items-center space-x-8">
            {navItems.map((item) => (
              <Link
                ...
                className={`nav-link font-sans text-sm font-semibold tracking-wide uppercase transition-all duration-300 cursor-pointer ${
                  activeSection === (item.href.includes('#') ? item.href.split('#')[1] : 'work')
                    ? 'text-[#ff758f]'
            ...
            <Link
              ...
              className="relative px-6 py-2.5 rounded-full overflow-hidden group border border-[#ff758f]/40 bg-[#ff758f]/10 text-white font-semibold text-xs tracking-wider uppercase transition-all duration-300 hover:border-[#ff758f] hover:shadow-[0_0_15px_rgba(255,117,143,0.3)] cursor-pointer"
            >
